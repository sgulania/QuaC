...
int main(int argc,char *args[]){
  PetscInt n_atoms,i,j,n_levels,n_seqgroups,max_seqgroupsize,val_init,pos1,pos2,dmpos,dmstdpos, depth,k,kk;
  ...
  PetscReal theta0[1000], phi0[1000], lambda0[1000];
  PetscReal theta1[1000], phi1[1000], lambda1[1000];
  ...
  // Define CNOT_ARP as a pulse
  // Target
  //1.0 * omega(t) * |r><1|
  add_ham_term_time_dep(qsys,1.0,&pulse_params[0],omega_arp,2,atoms[1][r],atoms[1][one]);
  //1.0 * omega(t) * |1><r|
  add_ham_term_time_dep(qsys,1.0,&pulse_params[0],omega_arp,2,atoms[1][one],atoms[1][r]);

  //1.0 * delta(t) * |r><r|
  //delta is time dependent in this case
  add_ham_term_time_dep(qsys,1.0,&pulse_params[0],delta_arp,2,atoms[1][r],atoms[1][r]);

  // Control 
  //1.0 * omega(t) * |r><1|
  add_ham_term_time_dep(qsys,1.0,&pulse_params[0],omega_arp,2,atoms[0][r],atoms[0][one]);
  //1.0 * omega(t) * |1><r|
  add_ham_term_time_dep(qsys,1.0,&pulse_params[0],omega_arp,2,atoms[0][one],atoms[0][r]);

  //1.0 * delta(t) * |r><r|
  //delta is time dependent in this case
  add_ham_term_time_dep(qsys,1.0,&pulse_params[0],delta_arp,2,atoms[0][r],atoms[0][r]);
  ...
  for( i = 0; i < depth; i = i + 1 ){
     theta0[i]  = -PETSC_PI + ((float)rand() /RAND_MAX)*2*PETSC_PI;
     phi0[i]    = -PETSC_PI + ((float)rand() /RAND_MAX)*2*PETSC_PI;
     lambda0[i] = -PETSC_PI + ((float)rand() /RAND_MAX)*2*PETSC_PI;
     theta1[i]  = -PETSC_PI + ((float)rand() /RAND_MAX)*2*PETSC_PI;
     phi1[i]    = -PETSC_PI + ((float)rand() /RAND_MAX)*2*PETSC_PI;
     lambda1[i] = -PETSC_PI + ((float)rand() /RAND_MAX)*2*PETSC_PI;
     //printf("theta, phi, lambda, %.20e %.20e %.20e \n", theta[i], phi[i], lambda[i] );
  }
  ...
  for( i = 0; i < depth; i = i + 1 ){
     ...
     //printf("theta, phi, lambda, %.20e %.20e %.20e \n", theta[i], phi[i], lambda[i] );
     //apply_1q_na_gate_to_qvec(dm,HADAMARD,atoms[1][one]); //apply H to atom 1	
     //apply_1q_na_gate_to_qvec(dm,HADAMARD,atoms[0][one]); //apply H to atom 1
     apply_1q_na_gate_to_qvec(dm,U3,atoms[0][one],theta0[i],phi0[i],lambda0[i]);
     apply_1q_na_gate_to_qvec(dm,U3,atoms[1][one],theta1[i],phi1[i],lambda1[i]);
...
/* //---------------------------------------------------------------------------------------------- */
  ...
  for( i = 0; i < depth; i = i + 1 ){
    ...
    add_gate_to_circuit_sys(&circ,time1+0.1,U3,0,theta0[i],phi0[i],lambda0[i]);
    add_gate_to_circuit_sys(&circ,time1+0.1,U3,1,theta1[i],phi1[i],lambda1[i]);
    add_gate_to_circuit_sys(&circ,time1+0.1,CZ_ARP,0,1);
  }
